...
type Kind = "passport" | "case_expiry" | "license";
type Row = {
  kind: Kind;
  title: string;
  subtitle: string;
...
const KIND_STYLE: Record<Kind, { bg: string; text: string; dot: string; label: string }> = {
  ...
  license:     { bg: "bg-[#EDE9FE]", text: "text-[#5B21B6]", dot: "bg-[#8B5CF6]", label: "License" },
};
...
export default async function RenewalsPage({ searchParams }: { searchParams: { kind?: string } }) {
  ...
  const [passportsRes, casesRes, licensesRes] = await Promise.all([
    ...
    supabase.from("companies").select("id, code, name, license_expires_at").is("deleted_at", null).not("license_expires_at", "is", null).lte("license_expires_at", in180Iso).order("license_expires_at", { ascending: true }),
  ]);
  ...
  for (const co of (licensesRes.data ?? []) as Array<{ id: string; code: string; name: string; license_expires_at: string }>) {
    rows.push({ kind: "license", title: co.name, subtitle: `${co.code} · Company license`, expires_at: co.license_expires_at, href: `/companies/${co.id}` });
  }
  ...
  const licensesCount = rows.filter((r) => r.kind === "license").length;
  ...
  return (
    <div className="max-w-[1100px]">
      ...
      <div className="flex items-center gap-1.5 mb-4 flex-wrap">
        ...
        <KindPill href="/renewals?kind=license" active={kindFilter === "license"} label={`Licenses (${licensesCount})`} dot={KIND_STYLE.license.dot} />
      </div>
